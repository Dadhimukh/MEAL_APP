...
const showingMeals = function displayingMeals() {
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=chicken`
        ...
        .then(data => {
            document.getElementById("displayingMeals").innerHTML = data.meals.map(
                (meal) => `
                            <div id="card" class="card mb-3" style="width: 20rem;">
                            <img src="${meal.strMealThumb}" class="card-img-top" alt="...">
                            <div class="card-body">
...
// it show's all meals card in main acording to search input value
function showMealList() {
>>>>>>> cbc949ceb9524c4953dd688b66ef827d77e4809b
...
=======
    meals.then(data => {
        if (data.meals) {
            data.meals.forEach((element) => {
                document.getElementById('searchResult').innerHTML = "Your Search Results are here";
                let isFav = false;
                for (let index = 0; index < arr.length; index++) {
                    if (arr[index] == element.idMeal) {
                        isFav = true;
